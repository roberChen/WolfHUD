...
elseif string.lower(RequiredScript) == "lib/managers/menu/blackmarketgui" then 
	...
	function BlackMarketGuiTabItem:init(main_panel, data, ...)
		if WolfHUD:getSetting("inventory_tab_names", "boolean") and tweak_data and WolfHUD.tweak_data and WolfHUD.tweak_data.inventory_names[data.category] and type(data.on_create_data[1]) == "number" then
			local id = math.floor((data.on_create_data[1] / #data.on_create_data) + 1)
			data.name_localized = WolfHUD.tweak_data.inventory_names[data.category][id] or nil
		end
